  ...
  (
    ...
    return (
      <div
        ...
        data-state={state}
        className={cn(
            'hidden md:flex flex-col h-screen sticky top-0 bg-sidebar border-r transition-[width] ease-linear',
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-8 w-8 transition-transform duration-200", state === 'collapsed' && 'rotate-180', className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
...
>(
  (
    ...
    const buttonContent = (
      <>
        {children}
        <span className={cn(state === 'collapsed' ? 'hidden' : 'inline-block truncate')}>
